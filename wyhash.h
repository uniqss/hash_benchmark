...
static inline uint64_t wyhash(const void *key, size_t len, uint64_t seed, const uint64_t *secret){
  ...
  if(_likely_(len<=16)){
      printf("kk ");
      if (_likely_(len >= 4)) { printf("ll "); a = (_wyr4(p) << 32) | _wyr4(p + ((len >> 3) << 2)); b = (_wyr4(p + len - 4) << 32) | _wyr4(p + len - 4 - ((len >> 3) << 2)); }
...
//a useful 64bit-64bit mix function to produwyce deterministic pseudo random numbers that can pass BigCrush and PractRand
